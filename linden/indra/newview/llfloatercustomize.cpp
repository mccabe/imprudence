...
void LLScrollingPanelParam::onSliderMoved(LLUICtrl* ctrl, void* userdata)
{
	...
	if (current_weight != new_weight )
	{
		...
		LLVOAvatar* avatar = gAgent.getAvatarObject();
		F32 avatar_size = (avatar->mBodySize.mV[VZ]) + (F32)0.195; //mBodySize is actually quite a bit off.
		int inches = (int)(avatar_size / .0254f); 
	    int feet = inches / 12; 
		inches %= 12; 
		
		floater_customize->getChild<LLTextBox>("HeightText")->setValue(llformat("%.2f", avatar_size) + "m");
		floater_customize->getChild<LLTextBox>("HeightText2")->setValue(llformat("%.2f", llround(feet)) + "' "
																	  + llformat("%.2f", llround(inches)) + "\"");
